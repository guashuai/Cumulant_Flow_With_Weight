...
//	initCanvas();
	...
	for(iFile=0;iFile<numFiles;iFile++){
		...
		fread(&EventNum,sizeof(int),1,fd);	
			while(!feof(fd)){
				fread(&RunNum,sizeof(int),1,fd);
				fread(&zdcUnAttenuatedEast,sizeof(double),1,fd);
				fread(&refMult,sizeof(int),1,fd);
				fread(&trigger,sizeof(int),1,fd);
				fread(&numberOfPrimaryVertices,sizeof(int),1,fd);
				fread(&zdcCoincidenceRate,sizeof(int),1,fd);
				fread(&zdcUnAttenuatedWest,sizeof(double),1,fd);
				fread(&VertexX,sizeof(float),1,fd);
				fread(&VertexY,sizeof(float),1,fd);
				fread(&VertexZ,sizeof(float),1,fd);
				fread(&TOFmult,sizeof(int),1,fd);
				fread(&vpdVz,sizeof(float),1,fd);
				fread(&Multiplicity,sizeof(int),1,fd);
				fread(&num_tracks,sizeof(int),1,fd); 
				fread(&cos2phiSum,sizeof(double),1,fd);	
				fread(&sin2phiSum,sizeof(double),1,fd);
				fread(&cos4phiSum,sizeof(double),1,fd);
				fread(&sin4phiSum,sizeof(double),1,fd);
